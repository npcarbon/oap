...
{
    $conn = condb();
    $chkChm = $conn->query('SELECT chem_id,chem_no FROM `chemicals` WHERE chem_no = "' . $_POST['chmNumber'] . '"');
    if (mysqli_num_rows($chkChm) == 0) {
        $insChm = "INSERT INTO `chemicals` (`chem_name`, `chem_producer`, `chem_code`, `chem_no`, `chem_status`) VALUES ";
        $insChm .= "('" . $_POST['chmName'] . "','" . $_POST['chmProducer'] . "','" . $_POST['chmCode'] . "','" . $_POST['chmNumber'] . "','" . $_POST['chmStatus'] . "')";
...
function addPackage()
{
    ...
    if (mysqli_num_rows($chkPack) < 1) {
        ...
        $insPack .= "('" . $_POST['pckProducer'] . "','" . $_POST['pckCode'] . "','" . $_POST['pckNumber'] . "','" . $_POST['pckVol'] . "')";
...
function addTruck()
{
    ...
    if (mysqli_num_rows($chkTruck) < 1) {
        $insTruck = "INSERT INTO `trucks` (`car_license`, `chasis_no`, `brand`, `model`, `type`) VALUES";
    ...
    if ($Chm_id) {
        if ($Pack_id) {
            if ($Truck_id) {
                ...
                $insJob .= '("' . $trader_id["trader_id"] . '","' . $Chm_id . '","' . $_POST['chmVolume'] . '","' . $_POST['chkDate'] . '",
                    "' . $_POST['chmQty'] . '","' . $Pack_id . '","' . $_POST['fromPlace'] . '","' . $_POST['toPlace'] . '","' . $_POST['from_date'] . '",
                    "' . $_POST['to_date'] . '","' . $_POST['jobController'] . '","' . $Truck_id . '","' . $_POST['spending'] . '")';
...
function getTrader($txtSearch)
{
    ...
    if ($_SESSION['role_id'] != 1) {
        ...
        while ($traders = $query->fetch_array(MYSQLI_ASSOC)) {
            error_reporting(error_reporting() & ~E_NOTICE);
            if ($traders['trader_id'] == $_GET["ID"]) {
                echo '
                    <form action="../function/database.php" method="post">
                    <tr>
                    <input type="text" hidden name="trader_id" value="' . $traders['trader_id'] . ' ">
                    <td> ' . $traders['trader_name'] . ' </td>
                    <td> ' . $traders['trader_code'] . ' </td>
                    <td>
                        <input type="text" class="form-control" name="address" value="' . $traders['address'] . '">
                    </td>
                    <td>
                        <input type="text" class="form-control" name="telephone" value="' . $traders['telephone'] . '">
                    </td>
                    <td>
                        <input type="text" class="form-control" name="fax" value="' . $traders['fax'] . '">
                    </td>
                    <td> ' . $traders['email'] . ' </td>
                    <td>
                        <input type="submit" class="btn btn-success btn-sm" value="บันทึก">';?>
                ...
                <td> ' . $traders['email'] . ' </td>
                <td>
                <a class="btn btn-primary btn-sm" href="' . $_SERVER["PHP_SELF"] . '?ID=' . $traders['trader_id'] . '" role="button"> แก้ไข</a>
                </td>
              </tr> ';
...
function getReport($license, $from_date, $to_date)
{
    ...
    traders.*
        ...
        INNER JOIN trucks ON trucks.truck_id = job_details.truck_id
        INNER JOIN packages ON packages.pack_id = job_details.pack_id
     WHERE traders.trader_id = " . $license . " AND ((job_details.formDate BETWEEN '" . $from_date . "' AND '" . $to_date . "') OR (job_details.atDate BETWEEN '" . $from_date . "' AND '" . $to_date . "'))");
    $i = 0;
...
{
    $conn = condb();
    $allow = $conn->query("UPDATE `traders` SET `status` = '1' WHERE `traders`.`trader_id` = $traderID");
    if ($allow) {
